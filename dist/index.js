...
function getCacheControl(providerMetadata) {
  ...
  const cacheControlValue = (anthropic2 == null ? void 0 : anthropic2.cacheControl) ?? (anthropic2 == null ? void 0 : anthropic2.cache_control);
...
async function convertToAnthropicMessagesPrompt({
  ...
  async function shouldEnableCitations(providerMetadata) {
    ...
    return ((_a = anthropicOptions == null ? void 0 : anthropicOptions.citations) == null ? void 0 : _a.enabled) ?? false;
  }
      ...
      case "assistant": {
        ...
        for (let j = 0; j < block.messages.length; j++) {
          ...
          for (let k = 0; k < content.length; k++) {
            ...
            const cacheControl = getCacheControl(part.providerOptions) ?? (isLastContentPart ? getCacheControl(message.providerOptions) : void 0);
            switch (part.type) {
              case "text": {
                anthropicContent.push({
                  type: "text",
...
var AnthropicMessagesLanguageModel = class {
  constructor(modelId, config) {
    ...
    this.config = config;
    this.generateId = config.generateId ?? import_provider_utils11.generateId;
  }
  supportsUrl(url) {
  ...
  get supportedUrls() {
    var _a, _b;
    return ((_b = (_a = this.config).supportedUrls) == null ? void 0 : _b.call(_a)) ?? {};
  }
  async getArgs({
    prompt,
    ...
    const { prompt: messagesPrompt, betas: messagesBetas } = await convertToAnthropicMessagesPrompt({
      prompt,
      sendReasoning: (anthropicOptions == null ? void 0 : anthropicOptions.sendReasoning) ?? true,
      warnings
    });
    const isThinking = ((_a = anthropicOptions == null ? void 0 : anthropicOptions.thinking) == null ? void 0 : _a.type) === "enabled";
    const thinkingBudget = (_b = anthropicOptions == null ? void 0 : anthropicOptions.thinking) == null ? void 0 : _b.budgetTokens;
    const baseArgs = {
      // model id:
      model: this.modelId,
  ...
  buildRequestUrl(isStreaming) {
    var _a, _b;
    return ((_b = (_a = this.config).buildRequestUrl) == null ? void 0 : _b.call(_a, this.config.baseURL, isStreaming)) ?? `${this.config.baseURL}/messages`;
  }
  transformRequestBody(args) {
    var _a, _b;
    return ((_b = (_a = this.config).transformRequestBody) == null ? void 0 : _b.call(_a, args)) ?? args;
  }
  extractCitationDocuments(prompt) {
    const isCitationPart = (part) => {
      ...
      return (citationsConfig == null ? void 0 : citationsConfig.enabled) ?? false;
    };
  ...
  async doGenerate(options) {
    ...
    return {
      ...
      usage: {
        ...
        cachedInputTokens: response.usage.cache_read_input_tokens ?? void 0
      },
      request: { body: args },
      response: {
          ...
          transform(chunk, controller) {
            ...
            switch (value.type) {
              ...
              case "message_start": {
                ...
                usage.cachedInputTokens = value.message.usage.cache_read_input_tokens ?? void 0;
                rawUsage = {
                  ...value.message.usage
                };
                ...
                controller.enqueue({
                  ...
                  id: value.message.id ?? void 0,
                  modelId: value.message.model ?? void 0
                });
                return;
              }
              case "message_delta": {
                usage.outputTokens = value.usage.output_tokens;
                usage.totalTokens = (usage.inputTokens ?? 0) + (value.usage.output_tokens ?? 0);
                finishReason = mapAnthropicStopReason({
...
function createAnthropic(options = {}) {
  const baseURL = (0, import_provider_utils20.withoutTrailingSlash)(options.baseURL) ?? "https://api.anthropic.com/v1";
  ...
  const createChatModel = (modelId) => new AnthropicMessagesLanguageModel(modelId, {
    ...
    headers: getHeaders,
    fetch: options.fetch,
    generateId: options.generateId ?? import_provider_utils20.generateId,
    supportedUrls: () => ({
      "image/*": [/^https?:\/\/.*$/]
    })
  });
